...
fn common_interchain_token_id(env: &Env, deploy_salt: BytesN<32>) -> BytesN<32> {
    env.crypto()
        .keccak256(&(PREFIX_INTERCHAIN_TOKEN_ID, Address::zero(env), deploy_salt).to_xdr(env))
        .into()
}
